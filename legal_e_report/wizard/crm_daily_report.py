...
class DailyCRMReport(models.TransientModel):

    _name = "daily.crm.report"
...
#        sheet.row(0).height = 256 * 4
        ...
        sheet.write_merge(0, 0,0,5,'CRM DAILY REPORT',style1)
        sheet.write_merge(1,2, 0,0,'CRM NAME',style1)
...
#            sheet.write(row,0,inv_id.client_service_manager_id.name)
        ...
        return {
             ...
             'url': '/opt/download?model=output&field=xls_output&id=%s&filename=CRM Daily Report.xls'%(attach_id.id),
             'target': 'new',
            } 
