...
class HrLoan(models.Model):
    ...
    def _default_employee_id(self):
        ...
        user_id = employee_obj.search([('user_id', '=', user_id)])
        return user_id
    
    #compute balance amt and paid amount
    @api.one
...
#check loan previous loan are loan are not paid employee can't create new loan 
    @api.model
    ...
    def unlink(self):
        for record in self:
            paid_loan_ids=record.loan_lines.filtered(lambda loan: loan.paid == True)
            if paid_loan_ids:
                raise ValidationError(_("This record can't be delete becuse of paid."))
            else:
...
# before aprove loan generate installment
    @api.multi
...
#count no of loan take employee
    @api.one
