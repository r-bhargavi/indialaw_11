    ...
    def button_confirm(self):
        if 'active_ids' in self._context:
            ...
            if len(self._context['active_ids'])>1:
                ...
                qry = "update project_task set remaining_hours=0.0, state='done', flg_message=True,progress=100, stage_id="+str(stage_id)+" where id ="+str(self._context['active_ids'][0])+""
            self.cr.execute(qry)
        return True
