...
class ConsolidatedBill(models.Model):
    ...
    def update_status(self):
        ...
        for bill_obj in self.browse(bill_ids):
                    count = 0
                    lines = []
...
#                        pass
        ...
        for rec in self:
            for inv in rec.case_invoice_lines:
                ...
                for line in inv.invoice_lines_court_proceedings_assignment:            
                    if line.amount:
                        amount = amount + line.amount
                self.env['case.sheet.invoice'].write([inv.id], {'amount_total_1':amount})
        return True
    ...
    payment_ids= fields.Many2many('account.move.line',compute='_compute_lines', string='Payments', groups='base.group_user')
    ...
    def cancel_bill(self):
        case_inv_ids = self.env['case.sheet.invoice'].search([('consolidated_id','=',self.ids[0])])
        for invoice in self.env['case.sheet.invoice'].browse(case_inv_ids):
            if invoice.invoice_id:
                self.env['account.invoice'].action_cancel([invoice.invoice_id.id])
    ...
    def confirm_bill(self):
        ...
        context.update({'consolidated_id':self.ids[0]})
        ...
        for case in obj.case_sheet_ids:
            ret = self.env['case.sheet'].invoice_case_sheet([case.id])
        ...
        if errstr:
            raise UserError(_('Warning'),_(errstr))
        if not obj.name:
            ...
            number = obj.name    
        self.write({'state':'confirm','name':number})
        return True
...
class ConsolidatedBillFollowup(models.Model):
    ...
    def bill_next_date_scheduler(self):
        ...
        invoice_ids = [follow_obj.bill_id.id for follow_obj in self.browse(followup_ids)]
        for invoice_obj in self.env['consolidated.bill'].browse(invoice_ids):
