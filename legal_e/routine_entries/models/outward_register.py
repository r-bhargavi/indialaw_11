...
class OutwardRegister(models.Model):
    ...
    @api.multi
    def _get_remainder_date(self,field):
        res = {}
        for reg in self:
            remaind = False
    ...
    state=fields.Selection([('new','New'),('done','Completed')],'State', default='new')
    ...
    def create(self,vals):
        ...
        if vals['datas_fname']:            
            attach_id = self.env['ir.attachment'].create({'name':vals['datas_fname'],'type':'binary','datas':vals['datas'],'user_id':self.env.user.id,'res_model':(obj.file_number and 'case.sheet' or 'outward.register'),'res_id':(obj.file_number and obj.file_number.id or retvals),'res_name':(obj.file_number and obj.file_number.name or obj.name)})
    ...
    def write(self, vals):
        ...
        if not line.attach_id and 'datas_fname' in vals:
            attach_id = self.env['ir.attachment'].create({'name':line.datas_fname,'type':'binary','datas':line.datas,'user_id':self.env.user.id,'res_model':'outward.register','res_id':line.id,'res_name':line.name})
